...
catalokey = InlineKeyboardMarkup(
	inline_keyboard = [
		...
		InlineKeyboardButton(text = 'Железо', callback_data = 'ferrum'),		   
		InlineKeyboardButton(text = 'Витамин С', callback_data = 'vitaC'),	 
		InlineKeyboardButton(text = 'Витамин D', callback_data = 'vitaD')]
	], resize_keyboard=True
)

'''инлайн клавиатура оформления покупки и возврата к каталогу'''
buykey = InlineKeyboardMarkup(
	inline_keyboard = [
...
async def get_buying_list(message):
    print(f'{message.from_user.username} вибирает витамины')
    ...
    for i in range(len(names)):
    	await message.answer(f'Название: {names[i]} | \nОписание: {descriptions[i]} | Цена: {prices*(i+1)}')
    	with open(img_names[i], 'rb') as img:
...
async def back_to_catalog(call):
        await call.message.answer('Выберите продукт для покупки:', reply_markup=catalokey)
        await call.answer()
        
        
@dp.callback_query_handler(text='magnesium')
async def buying_magnesium(call):
	await call.message.answer('Вы выбрали Магний. Хотите купить?', reply_markup = buykey)
	await call.answer()	

@dp.callback_query_handler(text='ferrum')
async def buying_ferrum(call):
	await call.message.answer('Вы выбрали Железо. Хотите купить?', reply_markup = buykey)
	await call.answer()
	
@dp.callback_query_handler(text='vitaC')
async def buying_vitaC(call):
...
async def product_buying(call):
	await call.message.answer(f'{message.from_user.username}, Вы успешно приобрели продукт!')
	await call.answer()	
